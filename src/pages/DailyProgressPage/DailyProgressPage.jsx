...
export default function DailyProgressPage() {
  ...
  const totalCalories = dailyCalorieSummary?.dailyCalorieIntake ?? 0;
  const caloriesPer = dailyCalorieSummary?.percentageCaloriesConsumed ?? 0;
  const caloriesLeft = dailyCalorieSummary?.remainingCalories ?? 0;

  const steps = 20000;
  const stepsPer = Math.round((100 * steps) / neededSteps) ?? 0;
  const stepsLeft = neededSteps - steps;
  //   console.log("stepsLeft :", stepsLeft);

  const sleep = 10;
  const sleepPer = Math.round((100 * sleep) / neededSleep) ?? 0;
  const sleepLeft = neededSleep - sleep;
    ...
    <div className={styles.cont}>
      ...
      <div className={styles.rightSideCont}>
          ...
          <p className={styles.metrixTitle}>
            ...
            <span
              className={clsx(
                ...
                (caloriesPer === 0 || caloriesPer > 100 || caloriesPer <= 50) &&
                  styles.badResult,
                caloriesPer < 80 && caloriesLeft > 50 && styles.mediumResult,
        ...
        <div className={styles.metrixCont}>
          ...
          <p className={styles.metrixTitle}>
            <span className={styles.metrixName}>Remaining steps to do :</span>
            <span
              className={clsx(
                styles.metrixQuantity,
                (stepsPer === 0 || stepsPer <= 50) && styles.badResult,
        ...
        <div className={styles.metrixCont}>
          ...
          diastolicR <= heartRateMax ? (
            <p className={styles.metrixTitle}>
              ...
              <span className={clsx(styles.metrixQuantity, styles.goodResult)}>
